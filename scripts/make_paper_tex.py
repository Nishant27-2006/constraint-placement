...
# , , , , , , , , , , , , , , , , , , , , , ,  bibliography
def write_bib():
    """Copy the cited entries verbatim out of the verified master bibliography."""
    ...
    for k in CITE_KEYS:
        ...
        if m:
            ...
            print(f"  WARNING: citation key not in references.bib: {k}")
    path = os.path.join(ROOT, "paper", "iclr_submission.bib")
    with open(path, "w") as f:
        f.write("\n\n".join(out) + "\n")
    print(f"  wrote paper/iclr_submission.bib ({len(out)} entries)")
...
def main():
    ...
    w("Scenario generation, sampling plausible 24-hour trajectories of load and "
      ...
      "risk-constrained dispatch. Diffusion models \\citep{ho2020denoising,song2021scoresde} "
      "and flow matching \\citep{lipman2023flow} produce high-fidelity samples. A "
      "sample that violates power balance, however, is inadmissible as input to an "
      "optimiser that assumes the balance holds.")
    w("")
    w("Exact constraint satisfaction is, by now, a solved problem in several "
      ...
      "the sample at inference \\citep{utkarsh2025pcfm}, complete the state from free "
      "coordinates \\citep{donti2021dc3}, or work in a nullspace basis. Each route is "
    ...
    w(r"\item \textbf{A codimension-controlled benchmark.} Three suites built from "
      ...
      r"\citep{pglib2021,zimmerman2011matpower}, differing in the fraction of the state "
    ...
    w(r"\paragraph{What we do not claim.} We claim no novelty for hard constraints in "
      r"flow matching: \citet{utkarsh2025pcfm} imposes arbitrary nonlinear constraints "
      r"zero-shot at inference. We claim no novelty for adaptive-step sampling "
      r"\citep{dormand1980family,chen2018neural}. Our method is not symplectic and is "
      r"unrelated to Hamiltonian generative flows \citep{holderrieth2024hamiltonian} or "
      r"Hamiltonian neural networks \citep{greydanus2019hamiltonian}; we report NFE and "
...
exactly. The contrast is therefore causal rather than correlational.""")
    ...
    w(f"We compare {nmeth} generators over {nseeds} seeds with an identical backbone, "
      ...
      "in the style of \\citep{utkarsh2025pcfm}; DC3-style completion \\citep{donti2021dc3}; "
      ...
      "\\citep{arjovsky2017wgan}, RealNVP-style normalising flows "
      "\\citep{dumas2022normflows,cramer2022pcanf} and DDPM \\citep{ho2020denoising}. "
      "Scoring uses strictly proper rules, energy score and variogram score "
      "\\citep{gneiting2007strictly,gneiting2008energyscore,scheuerer2015variogram}, "
...
the pattern, since \dataset{grid\_noflow} has the widest scale spread """
    ...
    w(r"\caption{\textbf{Cost regret tracks calibration, not feasibility.} Two-stage "
      r"stochastic unit commitment \citep{conejo2010decision}; commitment frozen on the "
...
the form in which they were written before the sweep.""")
    ...
    # , , , , , , , , , , , , , , , , , , , , -- bibliography
    w(r"\subsubsection*{Ethics statement}")
    w(r"""This work uses only public operating data published by the US Energy
...
precisely so that operators do not adopt exactness as a proxy for operational safety.""")
    w("")
    w(r"\subsubsection*{Reproducibility statement}")
    w(r"""All data are public and need no API key or licence. Section~\ref{sec:benchmark}
...
artifacts.""")
    w("")
    w(r"\bibliographystyle{iclr2027_conference}")
    w(r"\bibliography{iclr_submission}")
    w("")
    w(r"\appendix")
    w(r"\section{Proofs}\label{app:proofs}")
    w(r"""The setting is the one of Section~\ref{sec:routes}: $\mathcal C=\{x:Ax=b\}$
...
law $p_0=\Pi_{\mathcal C\#}\mathcal N(0,I)$, $x_t=(1-t)x_0+tx_1$ and $u=x_1-x_0$.""")
    w("")
    w(r"""\paragraph{Theorem~\ref{thm:affine}(i).} $\mathcal C$ is convex and contains
...
$t\in[0,1]$, and $Au=Ax_1-Ax_0=b-b=0$, so $u\in V$ almost surely.""")
    w(r"""\paragraph{Theorem~\ref{thm:affine}(ii).} $V$ is a closed linear subspace and
...
minimiser.""")
    w(r"""\paragraph{Theorem~\ref{thm:affine}(iii).} Write $v=\Pi_Vv+(I-\Pi_V)v$
...
identity, with equality exactly when $v$ is $V$-valued $p_t$-almost everywhere.""")
    w(r"""\paragraph{Theorem~\ref{thm:affine}(iv).} $Ax^{+}=Ax+h\sum_ib_i(Ak_i)=Ax=b$
because every stage satisfies $Ak_i=0$. Induction over steps gives the result for any
step size, stage count or order.""")
    w("")
    w(r"""\paragraph{Proposition~\ref{prop:manifold}.} Taylor-expanding $g$ along one
...
it in practice.""")
    ...
    w(r"\section{Full result tables}\label{app:full}")
    w(r"""Entries are means over seeds, generated from the run files by the same script
that produced every figure. Lower is better in every column except the coverage
columns, whose targets are 0.9 and 0.5.""")
    w("")
    APP_METRICS = [("energy_score", "ES"), ("crps", "CRPS"),
                   ("variogram_score", "VS"), ("sliced_w1", "SW$_1$"),
                   ("marginal_w1", "MW$_1$"), ("acf_err", "ACF"),
                   ("ramp_w1", "Ramp"), ("psd_err", "PSD"),
                   ("cov90", "cov$_{90}$"), ("cov50", "cov$_{50}$"),
    ...
    for s_ in SUITES:
        ...
        order = sorted(agg, key=lambda k: np.mean([x["energy_score"] for x in agg[k]]))
        w(r"\begin{table}[h]\centering\footnotesize")
        w(r"\resizebox{\textwidth}{!}{%")
        w(r"\begin{tabular}{l" + "r" * len(APP_METRICS) + r"}\toprule")
        w("method & " + " & ".join(m[1] for m in APP_METRICS) + r" \\\midrule")
        for mth in order:
            cells = []
            for key, _ in APP_METRICS:
                mu, _sd = ms([x.get(key) for x in agg[mth]])
                cells.append(num(mu))
            nm = texesc(mth)
            if mth == "HFM (ours)":
                nm = r"\textbf{" + nm + "}"
        ...
        w(f"\\caption{{Suite {TEXNAME[s_]}: all {len(agg)} methods over "
          f"{len({r['seed'] for r in rows_})} seeds.}}")
        w(r"\end{table}")
        w("")

    w(r"\section{Reproduction details}\label{app:repro}")
    w(r"""Data are downloaded by a single script and built deterministically. Days on
